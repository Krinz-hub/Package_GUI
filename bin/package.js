...
async function handleStatus() {
  ...
  if (isRunning && state) {
    ...
    console.log(`OS:                ${os.type()} ${os.release()} (${os.arch()})`);
    console.log(`Node.js:           ${process.version}`);
    console.log(`\nRun 'package' to start the local dashboard.\n`);
  }
  console.log('');
}
...
function handleLogs(args) {
  ...
  if (!fs.existsSync(logFile)) {
    console.log('No logs found yet. Start PACKAGE GUI with "package" to generate logs.');
    return;
  }
...
// -----------------------------------------------------------------------------
// Launch GUI Command (package / package gui / pakage)
// -----------------------------------------------------------------------------

async function handleLaunch(args) {
  const isDebug = args.includes('--debug') || process.env.PACKAGE_DEBUG === '1' || process.env.PAKAGE_DEBUG === '1';
  const noOpen = args.includes('--no-open') || process.env.PACKAGE_NO_OPEN === '1' || process.env.PAKAGE_NO_OPEN === '1';
